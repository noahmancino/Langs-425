...
*)
fun first_lower xs = if String.size xs > 0 then Char.isLower (String.sub(xs, 0)) else false
...
val longest_lowercase = List.foldl longest_lower_string ""

fun first_answer inner = fn xs =>
    case xs of
        [] => raise Match
    |   x::xs' => let val head = inner(x) in case head of
            NONE => first_answer inner xs'
        |   SOME x => x
        end;
